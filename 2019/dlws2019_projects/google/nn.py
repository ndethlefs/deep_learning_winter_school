...
for line in open('./google_review_ratings.txt', 'r'):
	l = line.replace("\t", "")
	l = line.split(',')
	if not '".26\t"'in line and not "2\t2." in line:
		inp = l[1:-2]
		outp = l[-2]	
		print(inp)
		x.append([round(float(i), 1) for i in inp])
		y.append(round(float(outp), 1))
	
#	y.append(int(round(float(outp),1)))
